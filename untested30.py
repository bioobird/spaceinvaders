...
def fire_bullet(rect, speed, is_enemy, is_boss=False, enemy_type=None, enemy=None, angle=0):
    ...
    size = bullet_sizes.get(enemy_type, (20, 20))  # Fetch sizes or default
    ...
    bullet = {
        ...
        'is_enemy': is_enemy,
        'is_boss': is_boss,
        'angle': angle,  # Store the angle in radians directly
    }
    ...
    if game_state == "start":
        ...
        for enemy in enemies[:]:
            ...
            if now - enemy['last_shot'] > enemy_fire_rate and enemy['health'] > 0:
                if enemy['is_boss']:
                    angles_in_degrees = [-30, 0, 30]  # Degrees to spread bullets
                    for angle in angles_in_degrees:
                        fire_bullet(
                            pygame.Rect(enemy['x'] + enemy['width'] // 2, enemy['y'] + enemy['height'], 20, 20),
                            boss_bullet_speed,
                            True,
                            is_boss=True,
        ...
        for bullet in (player_bullets[:] + enemy_bullets[:]):
            # For player bullets and straightforward enemy bullets, avoid sideward movement  
            if not bullet['is_enemy'] or (bullet['is_enemy'] and not bullet.get('is_boss')):
                # Direct upward movement for player bullets and direct downward for enemies
                dy = abs(bullet['speed'])
                bullet['rect'].y += dy if bullet['is_enemy'] else -dy
            else:
                # Apply angle-based movement for boss bullets
                dx = math.cos(bullet['angle']) * bullet['speed']
                dy = math.sin(bullet['angle']) * bullet['speed']
                bullet['rect'].x += dx
                bullet['rect'].y += dy
                
            # Check if bullets go out of bounds or hit something
