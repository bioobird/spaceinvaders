...
boss_bullets = []  # New list for boss bullets

enemy_bullets = []
enemy_bullet_speed = 3
boss_bullet_speed = 4  # Example value, adjust as needed
...
def generate_enemies(stage):
    ...
    for _ in range(enemy_count):
        enemy = {
            ...
            'mouth_state': 'closed',  # Add the 'mouth_state' attribute for all enemies
...
def fire_bullet(rect, speed, is_enemy, is_boss=False, enemy_type=None, enemy=None, angle=None):
    ...
    bullet_sizes = {
        ...
        'boss3': (50, 50),
        'boss4': (70, 70),
        'boss5': (90, 90),  # Define a unique or suitable size for boss5 bullets
    }

    bullet_size = bullet_sizes.get(enemy_type, (20, 20))  # Fetch sizes or default
    bullet_width, bullet_height = bullet_size

    # If an angle is provided, calculate the directional speeds dx and dy
    dx = dy = 0
    ...
    # Create a bullet dictionary
    bullet = {
        'rect': pygame.Rect(rect.centerx - bullet_width // 2, rect.y + rect.height // 2 - bullet_height // 2, bullet_width, bullet_height),
        'speed': speed,
        'dx': dx,
        'dy': dy,
        'is_enemy': is_enemy,
    ...
    # Add bullet to corresponding list
    if is_boss:
    ...
    # Toggle mouth state for enemies that can open/close mouth
    if enemy and 'mouth_state' in enemy:
        toggle_enemy_mouth_state(enemy)

def get_boss_bullet_angles(enemy_type):
    if enemy_type == 'boss3':
        ...
        angle1 = 0  # No angle offset for non-boss enemies
        angle2 = 0
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_state == "start":
                # Assuming start_rect is correctly positioned where the "Start Game" button is drawn
                start_rect = pygame.Rect(WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 - 50, 200, 50)
                if start_rect.collidepoint(mouse_pos):
                    ...
                    enemies.clear()
                    current_stage = 1
                    generate_enemies(current_stage)  # Ensure this function populates the enemies list correctly
                # Assuming quit_rect is correctly positioned where the "Quit Game" button is drawn
                quit_rect = pygame.Rect(WINDOW_WIDTH // 2 - 100, WINDOW_HEIGHT // 2 + 50, 200, 50)
                if quit_rect.collidepoint(mouse_pos):
                    running = False

        elif event.type == pygame.KEYDOWN:
            if game_state == "game" and event.key == pygame.K_SPACE and (pygame.time.get_ticks() - last_player_shot_time > player_bullet_fire_rate):
                last_player_shot_time = pygame.time.get_ticks()
    ...
    if game_state == "start":
        ...
        for enemy in enemies[:]:
            ...
            if now - enemy['last_shot'] > enemy_fire_rate and enemy['health'] > 0:
                # Distinguish between regular enemies and bosses for firing logic
                if enemy['is_boss']:
                    # Here, incorporate specific boss bullet firing logic if different from regular enemies
                    fire_enemy_bullets(enemy, now)  # Adjust this function as needed
                else:
                    fire_enemy_bullets(enemy, now)  # This can be generalized or split based on enemy type

        # Consider extracting the bullet update logic into a function for cleanliness
        update_bullets()

        # Draw all game entities and check for end-game conditions
        update_display("game")
        check_game_states()  # This might handle victory, game over conditions
