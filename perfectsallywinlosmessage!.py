...
def check_all_enemies_defeated():
    global running  # Make sure to declare 'running' as global
    if not enemies:
        if current_stage == 4:
            display_end_game_message("SALLY WINS")
            running = False  # Stop the game loop before exiting
        else:
            next_stage()
...
def explode(x, y, enemy_type):
    ...
    size = size_mapping.get(enemy_type, 30)  # Default explosion size
...
def display_end_game_message(message):
    ...
    pygame.time.wait(5000)  # Wait for 5 seconds after displaying the final message
...
while running:
    ...
    for enemy in enemies[:]:
        ...
        if enemy['x'] <= 0 or enemy['x'] + enemy['width'] >= WINDOW_WIDTH:
            enemy['speed'] *= -1
        if now - enemy['last_shot'] > enemy_fire_rate and enemy['health'] > 0:  # Ensure enemy is alive before firing
        ...
        if bullet['rect'].y < 0 or bullet['rect'].y > WINDOW_HEIGHT:
            ...
            if bullet['is_enemy']:
                if bullet['rect'].colliderect(pygame.Rect(spaceship_x, spaceship_y, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)):
                    ...
                    running = False  # This will exit the game loop
    ...
    if not running:
        break  # If running is set to False, break out of the while loop immediately
